...
foreach $KEY_REQUEST (sort {$a<=>$b} keys %HASH){
	...
	$REFERENCE="/path/to/" . "$KEY_REQUEST" . "/" . "acceptor.splice.sites/nucleotide.substitutions/candidates.mes";
	...
	while (<FILE3>){
		...
		$MUT{$NAME}{$POS}{$OBS}=1;
		$PATH="/path/to/" . "$KEY_REQUEST" . "/" . "acceptor.splice.sites/nucleotide.substitutions" . "/" . "$NAME" . "." . "$POS" . "." . "$OBS";
		...
		foreach $KEY1_TRANSCRIPT (sort {$a<=>$b} keys %{$EXON{$NAME}}){
			foreach $KEY1_NUMBER (sort {$a<=>$b} keys %{$EXON{$NAME}{$KEY1_TRANSCRIPT}}){
				...
				$NUMBER_exonPRE=$KEY1_NUMBER-1;
				if (exists $EXON{$NAME}{$KEY1_TRANSCRIPT}{$NUMBER_exonPRE}){	# The mutation could be located in the last exon of the gene so there would be no later intron
					$START_REC1B=$EXON{$NAME}{$KEY1_TRANSCRIPT}{$NUMBER_exonPRE}[0];
					$END_REC1B=$EXON{$NAME}{$KEY1_TRANSCRIPT}{$NUMBER_exonPRE}[1];
					if ($START_REC1A<=$POS && $POS<=$END_REC1A){	# $START_REC1A and $END_REC1A would define the exon limits where mutation of interest is located
						if ($STRAND==1){
							$START_intronPRE=$END_REC1B+1;
							$END_intronPRE=$START_REC1A;
							
							$FLAG++;
						}
						else{
							$START_intronPRE=$END_REC1A+1;
							$END_intronPRE=$START_REC1B;
							
							$FLAG++;
						}
						
						if ($FLAG!=0){
							$LENGTH_INTRONpre=$END_intronPRE-$START_intronPRE;
							$INTRON{$LENGTH_INTRONpre}=1;
				...
				if ($#inBASES>=2){	# It is the same to say "$#inPOS>=2". The read reaches 3 or more regions (then there are minimum 2 inner gaps)
					...
					for $A(0..$#inBASES){
						...
						if ($A==0){
							...
							$B=$A-1;
							$PRIOR_lastPOS=$endCOORD{$B}[0];
							# As previously noted, "$inPOS[$A]+1" corresponds to the real start position of an region aligned so we have to carry out a correction (-1) to determine the length of the adyacent gap 
							# $inLENGTH=($inPOS[$A]+1)-$PRIOR_lastPOS-1;
							$inLENGTH=$inPOS[$A]-$PRIOR_lastPOS;
							if (($PRIOR_lastPOS+1)<=$POS && $POS<=$inPOS[$A]){
								$gapLENGTH{$B}[0]=$inLENGTH;
							}
							if (($inPOS[$A]+1)<=$POS && $POS<=$lastPOS){							
								if ($inLENGTH==0){
									print FILE_print3 "$LINE4\n";
								...
								$exonCOORD{$A}=1;
								# The first intron has a value for the variable $B equal to 0							
								$gapLENGTH{$B}[0]=$inLENGTH;	# Lengths are storaged for the different inner gaps
					...
					if ($TICKET==0){	# The mutation is necessarily located in a gap
						foreach $KEY_B (sort {$a<=>$b} keys %gapLENGTH){
							$gapLENGTH_mut=$gapLENGTH{$KEY_B}[0];
						...
						foreach $KEY_A (sort {$a<=>$b} keys %exonCOORD){
							if ($KEY_A==0){	# The mutation is located in the first region aligned
								if ($STRAND==-1){
									$gapLENGTH_pre=$gapLENGTH{$KEY_A}[0];
									foreach $KEY_LENGTH_INTRON (sort {$a<=>$b} keys %INTRON){
										if ($KEY_LENGTH_INTRON==$gapLENGTH_pre){
											print FILE_print2 "$LINE4\n";
											goto END;
										}
									}
								...
								if ($STRAND==1){
									$KEY_B=$KEY_A-1;
									$gapLENGTH_pre=$gapLENGTH{$KEY_B}[0];
									foreach $KEY_LENGTH_INTRON (sort {$a<=>$b} keys %INTRON){
										if ($KEY_LENGTH_INTRON==$gapLENGTH_pre){
											print FILE_print2 "$LINE4\n";
											goto END;
