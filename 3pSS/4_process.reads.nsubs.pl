...
foreach $KEY_REQUEST (sort {$a<=>$b} keys %HASH1){
	...
	$PATH_FILE="/path/to/" . "$KEY_REQUEST" . "/" . "acceptor.splice.sites/nucleotide.substitutions/candidates.mes";
	...
	while (<FILE2>){
		...
		if (exists $HASH2{$NAME}{$POS}{$OBS}){	# Several 23-mer sequences for the same mutation could be often an indication of different cryptic splicing events
			goto LINE;
		}
		$HASH2{$NAME}{$POS}{$OBS}=1;
		$PATH_PRINT="/path/to/" . "$KEY_REQUEST" . "/" . "acceptor.splice.sites/nucleotide.substitutions" . "/" . "$NAME" . "." . "$POS" . "." . "$OBS";
