...
foreach $KEY_REQUEST (sort {$a<=>$b} keys %HASH){
	...
	$PATH="path/to/" . "$KEY_REQUEST" . "/" . "MES" . "/" . "donorss" . "/" . "nsubs";
	
	$MESfiles="/path/to/" . "maxentscan.program.files.list";
	...
	$COUNTmuts=0;
	$INPUT="/path/to/" . "$KEY_REQUEST" . "/" . "nsubs.sort.maf";
	open FILE, "<$INPUT";
	LINE:
	while (<FILE>){
		chomp;
		...
		if ($STRAND==-1){
			...
			qx{/path/to/samtools-0.1.19/samtools faidx /data/human_ref/hg19_sanger/hg19.fa $CHR:$START-$END > $output_minus_strand};
			...
			while (<FILE1>){
				...
				if ($A==4){
					...
					qx{perl /path/to/score5.pl $print_rv_comp >> $output_MES_ref};
					qx{perl /path/to/score5.pl $print_rv_comp_mut >> $output_MES_mut};
					
					for $D(0..($#CANONseq-8)){
						$END_MERGER=$END-$D;
						$START_MERGER=$END_MERGER-8;
						
						$mutPOSinSEQ=(8-$D)+1;	# Values of the variable $mutPOSinSEQ for genes located in the minus strand even working on its RV-COMP counterpart
			...
			while (<FILE2>){
				...
				if ($A==2){
					...
					qx{perl /path/to/score5.pl $print_fwd >> $output_MES_ref};
					qx{perl /path/to/score5.pl $print_fwd_mut >> $output_MES_mut};
					
					for $D(0..($#CANONseq-8)){
						$START_MERGER=$START+$D;
						$END_MERGER=$START_MERGER+8;
						
						$mutPOSinSEQ=(8-$D)+1;	# Values of the variable $mutPOSinSEQ for genes located in the plus strand
