...
while (<FILE1>){
	...
	while (<FILE2>){
		...
		$PATH_PRINT_PRE="/path/to/" . "$REQUEST" . "/" . "donor.splice.sites/nucleotide.substitutions" . "/" . "$NAME" . "." . "$POS" . "." . "$OBS" . "/" . "reads.cryptic.splicing.v4.nodup.min3_optB_mutpos_v2_PRE.txt";
					...
					if ($A>0){
						...
						if ((($inBASES[$B]+$inPOS[$B])+1)<=$POS && $POS<=$inPOS[$A]){
							if ($STRAND==1){
								...
								$ASS=$inPOS[$A]+1;
								if ((($lastPOS+1)<=$POS && $POS<=($lastPOS+6)) || ($ASS-50)<=$POS && $POS<=($ASS-15)){
									if (exists $HASH{$lastPOS}){
										$HASH{$lastPOS}[0]=$HASH{$lastPOS}[0]+1;
										$LENGTH=@{$HASH{$lastPOS}};
										$HASH{$lastPOS}[$LENGTH+1]=$ID;
								...
								$firstPOS=$inPOS[$A]+1;
								$ASS=$inBASES[$B]+$inPOS[$B];
								if ((($firstPOS-6)<=$POS && $POS<=($firstPOS-1)) || (($ASS+15)<=$POS && $POS<=($ASS+50))){
									if (exists $HASH{$firstPOS}){
										$HASH{$firstPOS}[0]=$HASH{$firstPOS}[0]+1;
										$LENGTH=@{$HASH{$firstPOS}};
										$HASH{$firstPOS}[$LENGTH+1]=$ID;
		...
		close FILE_PRINT;
		$PATH_PRINT="/path/to/" . "$REQUEST" . "/" . "donor.splice.sites/nucleotide.substitutions" . "/" . "$NAME" . "." . "$POS" . "." . "$OBS" . "/" . "reads.cryptic.splicing.v4.nodup.min3_optB_mutpos_v2.txt";
