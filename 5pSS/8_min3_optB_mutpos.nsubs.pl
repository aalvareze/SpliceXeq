...
# To be considered as SSM, the variant was required to be within the canonical splice-site; -3 to +6 positions at the 5'ss
...
while (<FILE1>){
	...
	while (<FILE2>){
		...
		$PATH_PRINT_PRE="/path/to/" . "$REQUEST" . "/" . "donor.splice.sites/nucleotide.substitutions" . "/" . "$NAME" . "." . "$POS" . "." . "$OBS" . "/" . "reads.cryptic.splicing.v4.nodup.min3_optB_mutpos_PRE.txt";
		open FILE_PRINT, ">$PATH_PRINT_PRE";
		
		$PATH_REF="/path/to/" . "$REQUEST" . "/" . "donor.splice.sites/nucleotide.substitutions" . "/" . "$NAME" . "." . "$POS" . "." . "$OBS" . "/" . "reads.cryptic.splicing.v4.nodup.txt";		
		$COUNT=0;
		...
		while (<FILE_REF>){
			...
			for $A(0..$#inBASES){
				if (($inPOS[$A]+1)<=$POS && $POS<=($inBASES[$A]+$inPOS[$A])){
					...
					if ($STRAND==-1 && $A>0){
						$firstPOS=$inPOS[$A]+1;
						if ($firstPOS<=$POS && $POS<=($firstPOS+2)){
					...
					if ($A>0){
						...
						if ((($inBASES[$B]+$inPOS[$B])+1)<=$POS && $POS<=$inPOS[$A]){
							if ($STRAND==1){
								...
								if (($lastPOS+1)<=$POS && $POS<=($lastPOS+6)){
									if (exists $HASH{$lastPOS}){
										$HASH{$lastPOS}[0]=$HASH{$lastPOS}[0]+1;
										$LENGTH=@{$HASH{$lastPOS}};
										$HASH{$lastPOS}[$LENGTH+1]=$ID;
								...
								$firstPOS=$inPOS[$A]+1;
								if (($firstPOS-6)<=$POS && $POS<=($firstPOS-1)){
									if (exists $HASH{$firstPOS}){
										$HASH{$firstPOS}[0]=$HASH{$firstPOS}[0]+1;
										$LENGTH=@{$HASH{$firstPOS}};
										$HASH{$firstPOS}[$LENGTH+1]=$ID;
		...
		close FILE_PRINT;
		$PATH_PRINT="/path/to/" . "$REQUEST" . "/" . "donor.splice.sites/nucleotide.substitutions" . "/" . "$NAME" . "." . "$POS" . "." . "$OBS" . "/" . "reads.cryptic.splicing.v4.nodup.min3_optB_mutpos.txt";
